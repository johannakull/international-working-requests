...
def check_eligibility():
    ...
    relevant_periods = []  # all periods that 12 months check needs to be done for
    ...
    print(f"\nBased on the proposed start date of {proposed_period_start_date}, the first relevant date is {first_relevant_date} and the last relevant date is {last_relevant_date}")
    ...
    with open("international_working_days.csv", newline="") as international_working_data:
        ...
        for row in dates_reader:
            ...
            if current_period_end_date >= first_relevant_date and current_period_start_date <= last_relevant_date:
                # if so, determine which days of the current period lie within the relevant range and add them to trimmed range - split this into separate function for readability
